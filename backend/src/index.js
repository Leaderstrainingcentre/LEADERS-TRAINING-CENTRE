...
const options = {
  definition: {
    openapi: "3.0.0",
      info: {
        version: "1.0.0",
        title: "LEADERS TRAINING CENTRE",
        description: "LEADERS TRAINING CENTRE Online REST API for Testing and Prototyping application. You can perform all major operations with your entities - create, delete and etc.",
      },
    servers: [
      {
...
app.use('/api/human_resources', passport.authenticate('jwt', {session: false}), human_resourcesRoutes);

app.use('/api/inventory', passport.authenticate('jwt', {session: false}), inventoryRoutes);

app.use('/api/machinery', passport.authenticate('jwt', {session: false}), machineryRoutes);

app.use('/api/quality_control', passport.authenticate('jwt', {session: false}), quality_controlRoutes);

app.use('/api/raw_materials', passport.authenticate('jwt', {session: false}), raw_materialsRoutes);
