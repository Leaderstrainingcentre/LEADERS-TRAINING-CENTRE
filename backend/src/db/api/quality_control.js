...
module.exports = class Quality_controlDBApi {
    ...
    static async bulkImport(data, options) {
        ...
        // Bulk create items
        const quality_control = await db.quality_control.bulkCreate(quality_controlData, { transaction });
    ...
    static async update(id, data, options) {
        ...
        const quality_control = await db.quality_control.findByPk(id, {}, {transaction});

        const updatePayload = {};

        if (data.check_point !== undefined) updatePayload.check_point = data.check_point;

        if (data.compliance !== undefined) updatePayload.compliance = data.compliance;
    ...
    static async findAll(filter, options) {
        ...
        try {
            const { rows, count } = await db.quality_control.findAndCountAll(queryOptions);
