...
const ListQuality_control = ({ quality_control, loading, onDelete, currentPage, numPages, onPageChange }: Props) => {
    ...
    return (
        <>
            <div className='relative overflow-x-auto p-4 space-y-4'>
                ...
                {!loading && quality_control.map((item) => (
                  <div key={item.id}>
                        ...
                        <div className={`flex rounded dark:bg-dark-900 border items-center overflow-hidden`}>
                          ...
                          >
                            ...
                            <div className={'flex-1 px-3'}>
                                ...
                                <p className={'line-clamp-2'}>{ dataFormatter.work_ordersOneListFormatter(item.work_order) }</p>
                            </div>

                            <div className={'flex-1 px-3'}>
                                <p className={'text-xs text-gray-500'}>Compliance</p>
                                <p className={'line-clamp-2'}>{ dataFormatter.booleanFormatter(item.compliance) }</p>
                            </div>
