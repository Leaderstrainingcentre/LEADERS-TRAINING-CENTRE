...
const InventoryTablesPage = () => {
  ...
  const [filters] = useState([{label: 'ProductName', title: 'product_name'},
    ...
    const getInventoryCSV = async () => {
        const response = await axios({url: '/inventory?filetype=csv', method: 'GET',responseType: 'blob'});
  ...
  return (
    <>
      ...
      <SectionMain>
        <SectionTitleLineWithButton icon={mdiChartTimelineVariant} title="Inventory" main>
        {''}
        </SectionTitleLineWithButton>
        <CardBox className='mb-6' cardBoxClassName='flex flex-wrap'>
          <BaseButton className={'mr-3'} href={'/inventory/inventory-new'} color='info' label='New Item' />
          ...
          <BaseButton
              ...
              onClick={addFilter}
          />
          <BaseButton className={'mr-3'} color='info' label='Download CSV' onClick={getInventoryCSV} />
            <BaseButton
              color='info'
              label='Upload CSV'
