...
const EditSuppliers = () => {
  ...
  useEffect(() => {
      if (typeof suppliers === 'object') {
          ...
          Object.keys(initVals).forEach(el => newInitialVal[el] = (suppliers)[el])
  ...
  return (
    <>
      ...
      <SectionMain>
        <SectionTitleLineWithButton icon={mdiChartTimelineVariant} title={'Edit suppliers'} main>
        {''}
    ...
    <FormField label="ContractTerms" hasTextareaHeight>
      ...
      >
          <DatePicker
              ...
              showTimeSelect
              selected={initialValues.delivery_schedule ?
                  new Date(
                      dayjs(initialValues.delivery_schedule).format('YYYY-MM-DD hh:mm'),
                  ) : null
              }
              onChange={(date) => setInitialValues({...initialValues, 'delivery_schedule': date})}
          />
      </FormField>
