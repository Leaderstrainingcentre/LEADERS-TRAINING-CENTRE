...
const EditMachinery = () => {
  ...
  useEffect(() => {
      if (typeof machinery === 'object') {
          ...
          Object.keys(initVals).forEach(el => newInitialVal[el] = (machinery)[el])
  ...
  return (
    <>
      ...
      <SectionMain>
        <SectionTitleLineWithButton icon={mdiChartTimelineVariant} title={'Edit machinery'} main>
        {''}
    ...
    >
      ...
      >
          <DatePicker
              ...
              showTimeSelect
              selected={initialValues.maintenance_schedule ?
                  new Date(
                      dayjs(initialValues.maintenance_schedule).format('YYYY-MM-DD hh:mm'),
                  ) : null
              }
              onChange={(date) => setInitialValues({...initialValues, 'maintenance_schedule': date})}
          />
      </FormField>
